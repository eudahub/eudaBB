...
1. `Nowe posty`
- zrobione; opis bieżącego zachowania jest w `docs/aktywnosc.md`
- w `TODO` zostają tylko rzeczy per-user, które mają się na tym oprzeć
...
4. `Tematy bez odpowiedzi` — zrobione
...
### Logika "nowości" a ignorowanie userów / klas

- Przy ignorowaniu pojedynczych userów nie wystarczy zwykłe `topic.last_post`.
- Dla widoków typu:
  - `Nowe posty`
  - `Nowe wątki`
  - `Nieprzeczytane`
  trzeba liczyć **ostatni widoczny post dla danego usera**, a nie tylko ostatni post obiektywnie w wątku.
- Jeśli najnowszy post w wątku napisał:
  - ignorowany user
  - albo user z ukrytej klasy spamu
  to taki post nie powinien sam z siebie powodować pojawienia się wątku na liście „nowych”.
- Wątek ma pojawić się jako nowy dopiero wtedy, gdy istnieje nowy **widoczny** post dla konkretnego usera.
- To oznacza, że przy pełnym ignorowaniu userów trzeba rozważyć osobne pojęcie:
  - `topic.last_visible_post_for_user`
  - albo przynajmniej helper wyznaczający „czy jest nowy widoczny post”.
- Z tego powodu kolejność wdrożenia powinna być ostrożna:
  1. najpierw `ignoruj wątek`
  2. potem ewentualnie `ignoruj usera`
  3. dopiero potem spiąć to z logiką `Nowe` / `Nieprzeczytane`

## Prywatne wiadomości (PM)

PM będą kompresowane, bez E2E szyfrowania.

### Antiflood PM

#### Bramka aktywności
- `active_days == 0` → brak możliwości wysyłania PM w ogóle
- `active_days >= 1` → PM dozwolone (z limitami poniżej)

#### Licznik nieprzerwanych wiadomości (per para nadawca→odbiorca)
Kluczowa reguła:

```
unresponded = liczba PM od mnie do X od czasu ostatniego PM od X do mnie
if unresponded >= pm_max_burst → zablokowany, aż X odpisze
```

Przykład dla `pm_max_burst = 2`:
```
Ja→X  unresponded=1  ✓
Ja→X  unresponded=2  ✓ (ostatni dozwolony)
Ja→X               → BLOKADA
X→Ja  (odpowiedź)  → reset unresponded=0
Ja→X  unresponded=1  ✓
Ja→X  unresponded=2  ✓
...
```
Dialog może osiągnąć dowolną liczbę wiadomości dziennie — muszą być przeplatane odpowiedziami.

#### Zimny reset po braku odpowiedzi
Jeśli X nigdy nie odpisał i minęło `pm_cold_reset_hours`:
- licznik unresponded jest resetowany — można znowu spróbować 2 PM
- zapobiega wiecznej blokadzie, ale spamer może próbować max raz dziennie

#### Globalny limit nowych rozmówców
- Max `pm_new_recipients_per_day` różnych "zimnych" rozmówców dziennie
  (osób, z którymi nigdy nie było wymiany PM)
- Zapobiega masowej wysyłce do wielu nieznajomych

#### Wartości domyślne — konfigurowalne przez roota w SiteConfig
```
pm_min_active_days      = 1   # bramka: active_days < N → brak PM
pm_max_burst            = 2   # max nieprzerwanych bez odpowiedzi
pm_cold_reset_hours     = 24  # po ilu h zimny licznik się resetuje
pm_new_recipients_per_day = 5 # max nowych rozmówców dziennie
```
...
Aktualna, wdrożona wersja wyszukiwarki jest opisana w `docs/wyszukiwarka.md`.

W `TODO` zostają tu tylko rzeczy jeszcze niewdrożone albo częściowo przygotowane.
...
- podstawowy filtr jest już zrobiony
- dalszy krok:
  - wyszukiwanie po samym ID filmu albo URL

7. **Tylko posty z linkami**
- podstawowy filtr jest już zrobiony
- dalszy krok:
  - wyszukiwanie po domenie
...
- podstawowe `combo` zależne od trybu jest już zrobione
- dalsze rozszerzenia:
  - kolejne typy specjalne
  - bardziej jednolity model filtrów dla postów i wątków
...
- podstawowy tryb jest już zrobiony
- dalsze kroki:
  - docelowo FTS także dla `Topic.title`, nie tylko obecne dopasowanie
  - dodatkowe filtry typów wątków poza samym `z ankietami`
  - ewentualne strojenie rankingu wyników
